...
namespace useQchangeweapon
{
    ...
    private class BindingData { public string quickBinding = string.Empty; }
        ...
        private void OnWeaponSelected(int weaponint)
        {
            if (NewWeapen_key != weaponint)
            {
                //检测对应武器栏是否有武器
                Slot? slot = null;
                switch(weaponint)
                {
                    case 0:
                        slot = CharacterMainControl.Main.GetSlot("PrimaryWeapon".GetHashCode());
                        break;
                    case 1:
                        slot = CharacterMainControl.Main.GetSlot("SecondaryWeapon".GetHashCode());
                        break;
                    case -1:
                        slot = CharacterMainControl.Main.GetSlot("MeleeWeapon".GetHashCode());
                        break;
                }
                if(slot == null) return;
        ...
        { 
            ...
            else if (LastWeapen_key >= 3 && LastWeapen_key <= 8) //3-8快捷栏物品切换
            {
